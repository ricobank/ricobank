...
abstract contract BalSetUp {
    ...
    VaultLike vault;
    WeightedPoolFactoryLike wpf;
    enum JoinKind { INIT, EXACT_TOKENS_IN_FOR_BPT_OUT, TOKEN_IN_FOR_EXACT_BPT_OUT }

    constructor() {
        vault = VaultLike(BAL_VAULT);
        wpf = WeightedPoolFactoryLike(BAL_W_P_F);
    }

    function create_pool(PoolArgs memory args) public returns(bytes32) {
        Asset memory a;
        Asset memory b;
